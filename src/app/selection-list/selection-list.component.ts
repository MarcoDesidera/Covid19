...
export class SelectionListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    hour >= 17
      ...
      : (this.TodayDate = year + '-' + month + '-' + (day - 1));
  ...
  parseSubmit(array: any) {
    if (this.form.value.orders == 'Stato') {
      ...
      var stateKeys: any = {
        ...
        totale_ospedalizzati: 'totale in ospedale',
        ...
        casi_testati: 'totale casi testati',
        ingressi_terapia_intensiva: 'in terapia intensiva',
        totale_positivi_test_molecolare: 'positivi al molecolare',
        totale_positivi_test_antigenico_rapido: 'positivi al rapido',
        tamponi_test_molecolare: 'tamponi molecolare',
        tamponi_test_antigenico_rapido: 'tamponi rapidi',
      };
      ...
      if (this.date != null) {
        for (
          let i = 0;
          i < array.length;
          i++ //scorro il file JSON
        ) {
          if (this.date == array[i].data.split('T')[0]) {
            //se le date del datepicker e quella dell'oggetto selezionato coincidono
            ...
            for (let key in array[i]) {
              ...
              array[i][key] == null
                ? null
                : (trow.innerHTML = stateKeys[key] + ': ' + array[i][key]);
            }
          }
        }
      } else {
        for (let key in array) {
          ...
          array[key] == ''
            ? null
            : (trow.innerHTML = stateKeys[key] + ': ' + array[key]);
        }
      }
    } else if (this.form.value.orders != 'Stato') {
      //se il valore di orders è diverso da "Stato" allora è una regione
      var regionKeys: any = {
        data: 'data',
        stato: 'stato',
        denominazione_regione: 'nome regione',
        ricoverati_con_sintomi: 'ricoverati con sintomi',
        terapia_intensiva: 'terapia intensiva',
        totale_ospedalizzati: 'totale in ospedale',
        isolamento_domiciliare: 'in isolamento domiciliare',
      ...
      if (this.date != null) {
        for (let i = 0; i < array.length; i++) {
          ...
          regione selezionata*/
          if (
            this.date == array[i].data.split('T')[0] &&
            array[i].denominazione_regione == this.form.value.orders
          ) {
            let regione = array[i];
            for (let key in array[i]) {
              let trow = document.createElement('TR');
              document.getElementById('outTable')?.appendChild(trow);
              regione[key] == null || regionKeys[key] == null
                ? null
                : (trow.innerHTML = regionKeys[key] + ': ' + regione[key]);
            }
          }
        }
      } else {
